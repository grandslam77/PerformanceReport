...
szerokoscObrazka   <- 50#(20+6*MiesiacAnalizy)
...
bazaMargin              <- bazaMargin[order(bazaMargin$DataAnalizy),]
...
bazaTurn                <- bazaTurn[order(bazaTurn$DataAnalizy),]
...
bazaQuantKol               <- pivot_wider(bazaQuant,names_from = NazwaWskaznika, values_from = Wartosc)
colnames(bazaQuantKol)[3]  <- "Current"
colnames(bazaQuantKol)[4]  <- "Previous"
bazaQuantKol$zmiana        <- round((bazaQuantKol$Current-bazaQuantKol$Previous)/bazaQuantKol$Previous*100,0)
bazaQuantKol$Abr           <- toupper(format(bazaQuantKol$DataAnalizy, "%b"))

bazaMarginKol              <- pivot_wider(bazaMargin,names_from = NazwaWskaznika, values_from = Wartosc)
colnames(bazaMarginKol)[3] <- "Current"
colnames(bazaMarginKol)[4] <- "Previous"
bazaMarginKol$zmiana       <- round((bazaMarginKol$Current-bazaMarginKol$Previous)/bazaMarginKol$Previous*100,0)
bazaMarginKol$Abr          <- toupper(format(bazaMarginKol$DataAnalizy, "%b"))

bazaTurnKol                <- pivot_wider(bazaTurn,names_from = NazwaWskaznika, values_from = Wartosc)
colnames(bazaTurnKol)[3]   <- "Current"
colnames(bazaTurnKol)[4]   <- "Previous"
bazaTurnKol$zmiana         <- round((bazaTurnKol$Current-bazaTurnKol$Previous)/bazaTurnKol$Previous*100,0)
bazaTurnKol$Abr            <- toupper(format(bazaTurnKol$DataAnalizy, "%b"))
...
wlasnyPorzadek <- c("JAN", "FEB", "MAR", "APR", "MAY", "JUN", "JUL", "AUG", "SEP", "OCT", "NOV", "DEC")
bazaQuantKol$Abr <- factor(bazaQuantKol$Abr, levels = wlasnyPorzadek)
...
all_negativeQ <- all(bazaQuantKol$zmiana < 0)
all_positiveQ <- all(bazaQuantKol$zmiana > 0)

# Utworzenie wektora vjust
vjustQuant <- if (all_negativeQ) {
  rep(1.6, nrow(bazaQuantKol))
} else if (all_positiveQ) {
  rep(-0,6, nrow(bazaQuantKol))
} else {
  ifelse(bazaQuantKol$zmiana < 0, -0.5, 1.5)
}



# jak zrobić by 0 ustawiało się zawsze w 
if(all(all_negativeQ,all_positiveQ)==FALSE){
  vjustQuant <-  ifelse(any(bazaQuantKol$zmiana < 0),1.6,-0.6)
}
# 
# QPU6HiT5Sfxkb0ggsQWsn6yym8mJ69FGoeNm9cdzMf0 kopecky77@gmail.com
...
all_negativeM <- all(bazaMarginKol$zmiana < 0)
all_positiveM <- all(bazaMarginKol$zmiana > 0)

# Utworzenie wektora vjust
vjustMarg <- if (all_negativeM) {
  rep(1.6, nrow(bazaMarginKol))
} else if (all_positiveM) {
  rep(-0.6, nrow(bazaMarginKol))
} else {
  ifelse(bazaMarginKol$zmiana < 0, -0.5, 1.5)
}

if(all(all_negativeM,all_positiveM)==FALSE){
  vjustMarg<-  ifelse(any(bazaMarginKol$zmiana < 0),1.6,-0.6)
}


all_negativeT <- all(bazaTurnKol$zmiana < 0)
all_positiveT <- all(bazaTurnKol$zmiana > 0)

# Utworzenie wektora vjust
vjustTurn <- if (all_negativeT) {
  rep(1.6, nrow(bazaTurnKol))
} else if (all_positiveT) {
  rep(-0.6, nrow(bazaTurnKol))
} else {
  ifelse(bazaTurnKol$zmiana < 0, -0.5, 1.5)
}

if(all(all_negativeT,all_positiveT)==FALSE){
  vjustTurn <- ifelse(any(bazaTurnKol$zmiana < 0),1.6,-0.6)
}



wykresKolumnowyQuant2<-ggplot(bazaQuantKol, aes(x = Abr, y = zmiana,fill="#A03FFF" )) +
 
   #geom_col(fill = "transparent", color = "#800080") +
  geom_col() +
  scale_fill_manual(values = "#A03FFF") +
  labs(title = "QUANTITY CHANGES (%Y/Y)", x = "", y = "") +
  theme(legend.position = "none") +
  geom_text(aes(y = 0, label = str_wrap(paste(zmiana,"%",sep=""), width = 10)),vjust = vjustQuant, size=2.5,colour=ifelse(bazaQuantKol$zmiana>0,"black","black")) + #,fontface="bold"
  theme(plot.title = element_text(hjust = 0.5, vjust = 0,size = 8.5,face="bold"), plot.margin = margin(t=0, unit="pt"),plot.caption = element_text(margin = margin(b = 5))) +
  theme(axis.line.y = element_blank(), axis.text.y = element_blank(), axis.ticks.y = element_blank()) +
  theme(axis.text.x = element_text(size = 7), axis.ticks.x = element_blank()) +
  scale_x_discrete(labels = function(x) str_wrap(x, width = 10)) +
  #theme(panel.grid = element_blank())+
  theme(panel.background = element_blank())


bazaMarginKol$Abr <- factor(bazaMarginKol$Abr, levels = wlasnyPorzadek)

wykresKolumnowyMargin2<-ggplot(bazaMarginKol, aes(x = Abr, y = zmiana,fill="#A03FFF" )) +   #bazaMarginKol
  #geom_col(fill = "transparent", color = "#800080") +
  geom_col() +
  scale_fill_manual(values = "#A03FFF") +
  labs(title = "MARGIN CHANGES (%Y/Y)", x = "", y = "") +
  theme(legend.position = "none") +
  geom_text(aes(y = 0, label = str_wrap(paste(zmiana,"%",sep=""), width = 10)),vjust = vjustMarg, size=2.5,colour=ifelse(bazaMarginKol$zmiana>0,"black","black")) + #,fontface="bold"
  theme(plot.title = element_text(hjust = 0.5, vjust = 0,size = 8.5,face="bold"), plot.margin = unit(c(0, 0.0, 0, 0), "cm"),plot.caption = element_text(margin = margin(b = 70))) +
  theme(axis.line.y = element_blank(), axis.text.y = element_blank(), axis.ticks.y = element_blank()) +
  theme(axis.text.x = element_text(size = 7), axis.ticks.x = element_blank()) +
  scale_x_discrete(labels = function(x) str_wrap(x, width = 10)) +
  #theme(panel.grid = element_blank())+
...
bazaTurnKol$Abr <- factor(bazaTurnKol$Abr, levels = wlasnyPorzadek)

wykresKolumnowyTurn2<-ggplot(bazaTurnKol, aes(x = Abr, y = zmiana,fill="#A03FFF" )) +   #bazaTurnKol
  #geom_col(fill = "transparent", color = "#800080") +
  geom_col() +
  scale_fill_manual(values = "#A03FFF") +
  labs(title = "TURNOVER CHANGES (%Y/Y)", x = "", y = "") +
  theme(legend.position = "none") +
  geom_text(aes(y = 0, label = str_wrap(paste(zmiana,"%",sep=""), width = 10)),vjust = vjustTurn, size=2.5,colour=ifelse(bazaTurnKol$zmiana>0,"black","black")) + #,fontface="bold"
  theme(plot.title = element_text(hjust = 0.5, vjust = 0,size = 8.5,face="bold"), plot.margin = margin(0,0,0,0)) +
  theme(axis.line.y = element_blank(), axis.text.y = element_blank(), axis.ticks.y = element_blank()) +
  theme(axis.text.x = element_text(size = 7), axis.ticks.x = element_blank()) +
  scale_x_discrete(labels = function(x) str_wrap(x, width = 10)) +
  #theme(panel.grid = element_blank())+
...
RazemTurn    <- WykresTurnov+wykresKolumnowyTurn2
RazemMargin  <- WykresMargin+wykresKolumnowyMargin2
RazemQuant   <- WykresQuant+wykresKolumnowyQuant2



#WykresZZ                  <- (WykresQuant/WykresMargin/WykresTurnov)#+(WykresQuantKol/WykresMarginKol/WykresTurnovKol) # +theme(plot.margin = unit(c(-1, -1, 0, 0), "cm"))
WykresZZ                  <- (RazemQuant/RazemMargin/RazemTurn)
