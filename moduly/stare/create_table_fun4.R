...
CreateTable <-function(tabela_wejsciowa){
  ...
  #tabela_wejsciowa2                             <- EKAT %>% group_by(Mapowanie,Region,SubRegion) %>% summarize(QActYear=sum(`Quantity KG TOTAL 2022`),QprevYear=sum(`Quantity KG TOTAL 2021`),GPACT=sum(`Margin EUR (adj) TOTAL 2022`),GPPREV=sum(`Margin EUR (adj) TOTAL 2021`),TURNACT=sum(`Turnover EUR (adj) TOTAL 2022`),TURNPREV=sum(`Turnover EUR (adj) TOTAL 2021`))
  #browser()
  ...
  tabela_wejsciowa                               <- tabela_wejsciowa %>% mutate_if(is.numeric, ~floor(.),.vars=vars(-nazwy_kolumn_grupujacych)) 
