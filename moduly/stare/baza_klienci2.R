...
NajstarszyOkreszBazaKlienci       <- max(baza_kliencii$MiesiacDanych) %>% print()
TabelaGreenNowe                   <- read_xlsx(here::here("sources",zestawDanych,"klienci_branże_2019.xlsm"),sheet = "Table",range="I15:X20000", col_names = TRUE,na = "NA")
TabelaGreenNowe                   <- TabelaGreenNowe %>% filter_all(any_vars(!is.na(.)))
...
DataNowychDanych                  <- unique(TabelaGreenNowe$MiesiacDanych) %>% print(.)
baza_kliencii2                    <- baza_kliencii[baza_kliencii$MiesiacDanych==floor_date(DataNowychDanych, unit = "month") - days(1),]
TabelaGreenNowe                   <- TabelaGreenNowe[,-c(7,8,11,12,15,16)]
TabelaGreenNowe                   <- TabelaGreenNowe %>% mutate_at(c(5:10), as.numeric)

TabelaGreenNowe                   <- left_join(TabelaGreenNowe,baza_kliencii2,by=c("Ids", "country"))
TabelaGreenNowe                   <- TabelaGreenNowe %>% mutate(across(where(is.numeric),replace_na, 0))
TabelaGreenNowe                   <- TabelaGreenNowe %>% mutate(NewAss=ifelse(PreviousAssignment.x!=PreviousAssignment.y & !is.na(PreviousAssignment.y),PreviousAssignment.x,"")) %>% filter(NewAss!="") %>% distinct()

length(unique(TabelaGreenNowe$Ids))


# if(interval(floor_date(DataNowychDanych, unit = "month"),floor_date(NajstarszyOkreszBazaKlienci,unit = "month")) %/% months(1)==0){
#   print("Zastępuję dane z poprzedniego miesiąca nową kopią.........")
#   
#   baza_kliencii <- baza_kliencii[baza_kliencii$MiesiacDanych!=DataNowychDanych,]
#  
#   Dane_doBazy   <-  
#   baza_kliencii <- rbind(baza_kliencii, Dane_doBazy
#   
# }else if(interval(floor_date(DataNowychDanych,unit = "month"),floor_date(NajstarszyOkreszBazaKlienci,unit = "month")) %/% months(1)>1){
#   print("Brakuje danych za poprzedni miesiąc.")
# 
# }else(interval(floor_date(DataNowychDanych,unit = "month"),floor_date(NajstarszyOkreszBazaKlienci,unit = "month")) %/% months(1)<0){
#   print("Próbujesz ponownie wgrać stare dane.")
# 
# }

# else{
#   print("Analizuję i dopisuję dane do bazy.........")
