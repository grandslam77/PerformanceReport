...
\definecolor{Ujemne}{RGB}{200, 21, 224}
\pagecolor{Brenntag}\afterpage{\nopagecolor}
\fontfamily{qhv}
<!-- \setmainfont[BoldFont = BrenntagSansTTNarrow-Bold.ttf]{BrenntagSansTTNarrow-Regular.ttf} -->
<!-- \setsansfont[BoldFont = BrenntagSansTTNarrow-Bold.ttf]{BrenntagSansTTNarrow-Regular.ttf} -->
\fontsize{12}{23}
<!-- \fontsize{12}{22} -->
...
\newgeometry{top=5.5cm,bottom=0.8cm,right=1cm,left=1.5cm}
...
\vspace{-2mm}
\hfill\textbf{\textcolor{black}{\small `r paste(MiesiacAnalizyTekst,RokAnalizy,sep=" ")`}}
...
```
```{r}
options(warn = -1) # Ukrycie wszystkich ostrzeżeń
```
...
\noindent Jan Kucala$^1$ \textit{"Maintained a positive trend in volumes as a region we’ve closed month with YY growth on volume by +6 713 MT +9\% YY; unfortunately, in GP we maintained YY loss trend, GP loss noted around whole CEE. In July we’ve noted \textbf{loss on GP by -6,4\%} -1,2 mio EUR, loss was noted in both divisions"}

\vspace{5mm}
\noindent Andras Szakolai$^2$ \textit{"Results with not assigned division in July was 457 TEUR on GP and this brings +414 TEUR YY growth for such N.A. transactions. \textbf{YTD for CEE – closed with loss on GP by -14,7 mio EUR/-10\%}. YTD results by divisions: BES loss on GP by -1,7 mio EUR (-2\%), BSP with loss"}
...
<!-- \columnratio{0.33} -->
<!-- \begin{paracol}{2} -->
<!--   \textbf{STORY OF THE MONTH - TOTAL} -->
<!--     \newline -->
<!--     \vspace{2mm} -->
<!-- No. of working days – the same in JULY.2023 vs JULY.2022 (21 WD) Loss on volume due to close activity in Russia in July: -214 MT (for GP was noted growth by +47TEUR due to fact of prev.year results with minus margin = extraordinary currency rates due to war crisis business shutdown processes in progress last year) In July 2023 we maintained a positive trend in volumes as a region we’ve closed month with YY growth on volume by +6 713 MT +9% YY; unfortunately, in GP we maintained YY loss trend, GP loss noted around whole CEE. In July we’ve noted loss on GP by -6,4% -1,2 mio EUR, loss was noted in both divisions but most of it – generated by BSP: BES -484 TEUR (-5%), BSP -1,2 mio EUR (-13%). Results with not assigned division in July was 457 TEUR on GP and this brings +414 TEUR YY growth for such N.A. transactions. YTD for CEE – closed with loss on GP by -14,7 mio EUR/-10%. YTD results by divisions: BES loss on GP by -1,7 mio EUR (-2%), BSP with loss -15 mio EUR (-20%).As mentioned, reverse to GP, on volumes we’ve noted YY growth in JULY overall CEE with growth by +6 713 MT/+8,7%. Growth noted for both Divisions: BES Division by +5 153 MT (+10%), BSP with loss by +464 MT (+2%). N.A. division +1 097 MT YY. -->
<!--     \newline -->
<!--   \switchcolumn -->
