...
TopBottom <- function(TabelaWejsciowaT,Przedrostek){
  ...
  Margin_plot<-ggplot(tabelaWejsciowa1_PLOT, aes(x = MaterialGroup, y = ZMIANA,fill="#800080" )) +
    ...
    geom_text(aes(y = 0, label = str_wrap(paste(ZMIANA,"%",sep=""), width = 10)),vjust = ifelse(tabelaWejsciowa1_PLOT$ZMIANA>0,1.5,-0.5), size=2.2,colour=ifelse(tabelaWejsciowa1_PLOT$ZMIANA>0,"black","red")) + #,fontface="bold"
  ...
  Quantity_plot<-ggplot(tabelaWejsciowa2_PLOT, aes(x = MaterialGroup, y = ZMIANA,fill="#4B0082" )) +
    geom_col() +
    scale_fill_manual(values = "#4B0082") +
    labs(title = "Monthly Quantity Difference (%Y/Y)", x = "", y = "") +
    theme(legend.position = "none")+
    geom_text(aes(y = 0, label = str_wrap(paste(ZMIANA,"%",sep=""), width = 10)),vjust = ifelse(tabelaWejsciowa2_PLOT$ZMIANA>0,1.5,-0.5), size=2.2,colour=ifelse(tabelaWejsciowa2_PLOT$ZMIANA>0,"black","red"))+   #,fontface="bold"
    theme(plot.title = element_text(hjust = 0.5, vjust = -0.5,size = 6.5,face="bold"))+
    theme(axis.line.y = element_blank(), axis.text.y = element_blank(), axis.ticks.y = element_blank())+
    theme(axis.line.x = element_blank(), axis.text.x = element_text(size = 5,family = "Courier New"), axis.ticks.x = element_blank())+
    scale_x_discrete(labels = function(x) str_wrap(x, width = 10))+
    #theme(panel.grid = element_blank())+
    theme(panel.background = element_blank())
  #theme(panel.grid.major.x=element_line(linetype = "dashed",colour = "light gray"))
