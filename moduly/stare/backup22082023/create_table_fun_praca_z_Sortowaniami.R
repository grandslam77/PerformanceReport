...
CreateTable <-function(tabela_wejsciowa,nazwa_tabeli="", pomocnicza){
  ...
  # if(nazwa_tabeli %in% c("TOP20GP")){#},"TOP20GPYTD","BOTTOM20GP","BOTTOM20GP_YTD","BOTTOM20QUANT","BOTTOM20QUANT_YTD","TOP20QUANT","TOP20QUANT_YTD")){
  ...
  # tutaj ustawia się kolejność kolumn więc nie ważne w jakiej kolejności kolumny wejdą do funkcji, ważne
...
# zasady sortowania tabel 
  ...
  if(nazwa_tabeli %in% c("TOP20GP","TOP20GPYTD")){
    ...
    }else if(nazwa_tabeli %in% c("TOP20QUANT","TOP20QUANT_YTD")){
  ...
  if(nazwa_tabeli %in% c("MainTotalYTD","MainTotal","BBSM","BBSM_YTD","BBS_EI","BBS_EI_YTD","BBS_KEY_YTD")){
    tabela_wejsciowa                        <- tabela_wejsciowa[order(tabela_wejsciowa[,1]),] 
  }
  
  #df[order(df[,1]),]   sortowanie po danej kolumnie
  ...
  if(nazwa_tabeli %in% c("MAIN_ESSP","MAIN_ESSP_YTD","BESBSP_PD_MT","BESBSP_PD_MT_YTD","MAIN_ESSP_T","MAIN_ESSP_TY","BESBSP_IRPD","BESBSP_IRPD_YTD","BESBSP_PDICCPM","BESBSP_PDICCPM_YTD","BESBSP_ICCPM","BESBSP_ICCPM_YTD","BESBSP_DI","BESBSP_DI_YTD")){
  ...
  if (ilosc_poziomow_grupowania==2){
    ...
    for(poziomgrupowania1 in PoziomGrupowania1){
     ...
     # tabela_wejsciowa                      <- tabela_wejsciowa[order(colnames(tabela_wejsciowa)[1]),] 
...
#### 4 poziomy grupowania ------------------------------------------------------------------------------------------
  ...
  if(nazwa_tabeli %in% c("BESBSP_DI","BESBSP_DI_YTD")){
    tabela_wejsciowa$MapowanieMD[tabela_wejsciowa$MapowanieMD=="N|A type"] <-"ŻN|A type"
  }
  ...
  if(nazwa_tabeli %in% c("BESBSP_DI","BESBSP_DI_YTD")){
    tabela_wejsciowa$MapowanieMD[tabela_wejsciowa$MapowanieMD=="ŻN|A type"] <-"N|A type"
  }
...
tab  <- CreateTable(BESBSP_DI,nazwa_tabeli = "BESBSP_DI")
#
asd  <- tab[[1]]
