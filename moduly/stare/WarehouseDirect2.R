...
MainSourceWhDir               <- read_xlsx(here::here("sources","DEC_NEW_Perform.Rep_2019_YTD_BBS_SR_WT_CL_TOP20.xlsm"),sheet = "BES_BSP",range="I16:y60000", col_names = TRUE,na = "NA")
...
colnames(MAIN_WhDir_MTH)[12]     <- Naglowek1
colnames(MAIN_WhDir_MTH)[13]     <- Naglowek2
colnames(MAIN_WhDir_MTH)[14]     <- Naglowek3
colnames(MAIN_WhDir_MTH)[15]     <- Naglowek4
colnames(MAIN_WhDir_MTH)[16]     <- Naglowek5
colnames(MAIN_WhDir_MTH)[17]     <- Naglowek6


MAIN_WhDir_MTH                   <- MAIN_WhDir_MTH %>% filter(`Calendar month`==MiesiacAnalizy)
MAIN_WhDir_MTH                   <- MAIN_WhDir_MTH %>% select(19,20,12,13,14,15,16,17,18)
...
Total <- data.frame(Region="Total CEE",SubRegion="Total CEE", WhsDirComm="Total CEE thereof",
                    ...
                    TurnDiffYYPER     = 0,
                    GPMTACT           = sum(MAIN_WhDir_MTH$GPMTACT),
                    GPMTPREV          = sum(MAIN_WhDir_MTH$GPMTPREV),
                    GPMTDIFF          = 0,
                    GPMTDIFFPER       = 0)
...
MAIN_WhDir_MTH[is.na(MAIN_WhDir_MTH)]    <- ""
...
colnames(MAIN_WhDir_YTD)[13]     <- Naglowek1
colnames(MAIN_WhDir_YTD)[14]     <- Naglowek2
colnames(MAIN_WhDir_YTD)[15]     <- Naglowek3
colnames(MAIN_WhDir_YTD)[16]     <- Naglowek4
colnames(MAIN_WhDir_YTD)[17]     <- Naglowek5
colnames(MAIN_WhDir_YTD)[18]     <- Naglowek6

MAIN_WhDir_YTD                  <- MAIN_WhDir_YTD%>% filter(`Calendar month` %in% 1:MiesiacAnalizy)
MAIN_WhDir_YTD                  <- MAIN_WhDir_YTD%>% select(19,20,12,13,14,15,16,17,18)
...
Total <- data.frame(Region="Total CEE",SubRegion="Total CEE", WhsDirComm="Total CEE thereof",
                    ...
                    TurnDiffYYPER     = 0,
                    GPMTACT           = sum(MAIN_WhDir_YTD$GPMTACT),
                    GPMTPREV          = sum(MAIN_WhDir_YTD$GPMTPREV),
                    GPMTDIFF          = 0,
                    GPMTDIFFPER       = 0)
...
MAIN_WhDir_YTD[is.na(MAIN_WhDir_YTD)]    <- ""
